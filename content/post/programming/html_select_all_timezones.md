---
title: HTML Select List for All Timezones
...
### Abbreviated HTML Select of Timezones

A smaller HTML select with timezones which maybe a good starting point in the USA.
...
What I especially liked about the `value` field here is it standard across platforms and programming languages. The reason is because this the `value` field if from the [tz database](https://en.wikipedia.org/wiki/List_of_tz_database_time_zones). Other `value`s you will see for the HTML `option` have offsets and all sorts of different ideas. These usually fail because when dealing with time one has to account for the dreaded Daylight Savings Time.

I will now belabor the point of how going with the `tz database` value is the way to go by showing it in many different programming languages and finally in Unix.
...
### Java Code to Create a HTML Select of All Timezones

Running this code will pull from the Java SDK the timezones in the `tz database`. We can use it go generate an HTML select.

{{< highlight java >}}
import java.util.TimeZone;

public class JavaScriptTimeZoneSelect {

  public static void main(String[] args) {
    String[] tzStrings = TimeZone.getAvailableIDs();
    System.out.println("<select id=\"timezone\" name=\"timezone\">");
    for(int i=0; i<tzStrings.length; i++) {
      TimeZone tz = TimeZone.getTimeZone(tzStrings[i]);
      String displayName = tz.getDisplayName();
      String name = tzStrings[i];
      Integer offset = tz.getRawOffset();
      Double offsetF = offset.doubleValue();
      offsetF = offsetF / 1000.0 / 60.0 / 60.0;

      String str = "GMT";
      if(offset>=0) str = "GMT+";

      Long offsetHours =  Math.round(Math.floor(offsetF));
      Double offsetMinsF = offsetF - Math.floor(offsetF);
      Long offsetMins = Math.round(Math.floor(offsetMinsF*60.0));

      String f = "  <option value=\"%s\">%s %s%d:%02d</option>";
      System.out.println(String.format(f,name,name,str,offsetHours,offsetMins));
    }
...
### Full List of HTML Select of Timezones in Action

Behold the full, overwhelming, crazy, way-to-long list of timezones in a single HTML Select. It gives you of something to work with and trim down.
...
### Full List of HTML Select of Timezones
{{< highlight html >}}
<select id="timezone" name="timezone" class="custom-select">
  <option value="Africa/Abidjan">Africa/Abidjan GMT+0:00</option>
  <option value="Africa/Accra">Africa/Accra GMT+0:00</option>
