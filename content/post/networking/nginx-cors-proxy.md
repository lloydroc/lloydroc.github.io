...
CORS works on 3 levels: protocol, origin and port. That is if a browser is going to make a request to a server then the protocol, url and port must be the same as the page that the browser loaded. For example if from a page on the browser hosted on `https://lloydroc.github.io:1123/example.html` if Javascript tries to make a request to say `http://lloydroc.github.io` then it would not be allowed since both the prototcol - `http` instead of `https` and port `80` instead of `1123` would not be allowed. Also, the Javascript cannot make a call to an origin other than `lloydroc.github.io`. This is all assuming, that CORS isn't enabled. When CORS is enabled we can get a lot of flexibility to allow clients to access back-ends many different origins.
